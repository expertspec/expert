...
def test_audio_analysis(detector):
    div_aud_agg, full_aud_agg = detector.get_audio_state(stamps=detector.stamps["SPEAKER_00"])

    assert type(div_aud_agg) == list
    assert len(div_aud_agg) > 1
...
def test_text_analysis(detector, fragments):
    ...
    assert len(div_text_agg) > 1
    assert type(full_text_agg) == dict
    assert full_text_agg["toxic_part"] == full_text_agg["toxic_part"]
    assert full_text_agg["depreciation_part"] == full_text_agg["depreciation_part"]
...
def test_anger_analysis(detector):
    ...
    assert len(div_agg["video"]) > 1
    assert type(div_agg["audio"]) == list
    assert len(div_agg["audio"]) > 1
    assert type(div_agg["text"]) == list
    assert len(div_agg["text"]) > 1
