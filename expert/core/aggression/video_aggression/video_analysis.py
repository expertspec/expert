...
class VideoAggression:
    ...
    def __init__(
        ...
        self.features = self.features.drop_duplicates(
            subset=["time_sec"]).sort_values(by="time_sec").reset_index(drop=True)
    ...
    def mesh_detection(self) -> List:
        ...
        for row in range(len(self.features)):
            ...
            if len(face_mesh) != 0:
                head_vector = torch.from_numpy(preprocess_face_vector(
                    face_mesh, head_indexes=HEAD_INDEXES))
                angle_prediction = self.anglenet(head_vector.to(
                    self._device).float()).detach().cpu().numpy()
    ...
    def get_report(self) -> Tuple[List, List, str]:
        ...
        quantile_25, quantile_75 = np.quantile(
            data["upp_coef"], 0.25), np.quantile(data["upp_coef"], 0.75)
        quantile_15, quantile_85 = np.quantile(
            data["upp_coef"], 0.15), np.quantile(data["upp_coef"], 0.85)
        ...
        quantile_25, quantile_75 = np.quantile(
            data["low_coef"], 0.25), np.quantile(data["low_coef"], 0.75)
        quantile_15, quantile_85 = np.quantile(
            data["low_coef"], 0.15), np.quantile(data["low_coef"], 0.85)
        ...
        full_vid_agg.append({
            "video_path": self.video_path,
            "uppface_activity": float(data[data["upp_motion"] != 0]["upp_motion"].count() / len(data)),
            "low_face_activity": float(data[data["low_motion"] != 0]["low_motion"].count() / len(data)),
            "rapid_activity": float(data[data["sum_motion"] > 2]["sum_motion"].count() / len(data)),
            "rotate_activity": float(data[data["rotations"] != 0]["rotations"].count() / len(data)),
        })
