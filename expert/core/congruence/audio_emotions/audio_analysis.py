...
class AudioAnalysis:
    ...
    def __init__(
        ...
        """
        Args:
            video_path (str | PathLike): Path to video.
            stamps (_type_, optional): Timestamps extracted from the infer file.
            speaker (str, optional): Name of the speaker. Defaults to 'SPEAKER_00'.
    ...
    def predict(self, duration=10):
        """Create report with information of the key emotions.
        
        Args:
            duration (int, optional): Window size. Defaults to 10.
        """
        ...
        if self.stamps[self.speaker]:
            for stamp in self.stamps[self.speaker]:
                ...
                self.chunks = self._chunkizer(duration, fragment.numpy(), self.sr)
                ...
                for num, chunk in enumerate(self.chunks):
                    ...
                    for i in range(len(self.chunk_parts)):
                        ...
                        mfcc = torchaudio.transforms.MFCC(sample_rate=self.sr, n_mfcc=13)(w)
                        
                        mfcc = np.transpose(mfcc.numpy(), (1, 2, 0))
                        mfcc = np.transpose(mfcc, (2, 0, 1)).astype(np.float32)
                        
                        self.test.append(torch.tensor(mfcc, dtype=torch.float).to(self._device))
                    ...
                    for i in range(len(self.test)):
                        ...
                        lim_emotions = softmax(torch.Tensor([[logits[0], logits[3], logits[2]]]))[0].numpy()
