...
class VolumeAnalyzer:
    def __init__(
        self, chunk_length, normalization=True, frame_size=512, signal=None, sr=22050, voice=None, path_to_audio=False
    ):
        ...
        if voice:  # Если переданы параметры частот относящихся к речевым
            ...
            self.voice = librosa.yin(
                self.y, frame_length=2048, fmin=librosa.note_to_hz("C2"), fmax=librosa.note_to_hz("A5")
            )  # Выделение голоса
            for i in range(len(self.voice)):
                if self.voice[i] > librosa.note_to_hz("A5") or self.voice[i] < librosa.note_to_hz("C2"):
                    self.voice[i] = np.nan
        ...
        self.noise_lvl = []  # Доля отброшенных фрагментов (не относящихся к голосовому диапазону)
        self.ampl_series = []
        self.silence = []  # Категориальный признак, указывающий на то, является ли фрагмент тишиной

    # Делит аудио файл(фрагменты) на чанки
    def chunkizer(self, audio, sr):
        num_chunks = ceil(librosa.get_duration(y=audio, sr=sr) / self.chunk_length)
        for i in range(num_chunks):
            self.chunks.append(audio[i * self.chunk_length * sr : (i + 1) * self.chunk_length * sr])
        return self.chunks
    ...
    def analyze_fragment(self):
        if self.duration > self.chunk_length:
            ...
            for chunk in self.chunks:
                ...
                for i in range(0, len(chunk), self.frame_size):
                    ...
                    amplitude_envelope.append(current)  # Запись амплитудных значений
        ...
        for i in range(len(self.voice_vols)):
            new_fragments_param = []  # Заменим значения на значения актуальные только для голоса
            ampl_len = len(self.voice_vols[i])  # Кол-во амплитудных значений фрагмента
            ...
            self.noise_lvl.append(round(np.nansum(self.voice_vols[i]) / len(self.voice_vols[i]), 5))
        ...
        for i in range(len(self.voice_vols)):
            ...
            for j in self.voice_vols[i]:
                if (
                    ...
                    if j <= self.average_vols[i] - abs(np.std(self.voice_vols[i])) / 2:  # Разделил на 2, так как
