...
class AntiplagiatClient:
    ...
    def simple_check(
        ...
        if status.Status == "Failed":
            logger.error(
                f"An error occurred while validating the document {filename}: {status.FailDetails}"
            )
        ...
        logger.info(
            f"Author Surname={fullreport.Attributes.DocumentDescription.Authors.AuthorName[0].Surname} "
        ...
        result.author.surname = (
            fullreport.Attributes.DocumentDescription.Authors.AuthorName[
                0
            ].Surname
        )
        result.author.othernames = (
            fullreport.Attributes.DocumentDescription.Authors.AuthorName[
                0
            ].OtherNames
        )
        result.author.custom_id = (
            fullreport.Attributes.DocumentDescription.Authors.AuthorName[
                0
            ].PersonIDs.CustomID
        )
    ...
    def get_verification_report_pdf(
        ...
        try:
            reportOptions = self.client.factory.create(
                "VerificationReportOptions"
            )
            reportOptions.Author = (
                author  # Full name of the author of the work.
            )
            reportOptions.Department = department  # Faculty (department).
            reportOptions.ShortReport = (
                True  # If a link to the summary required (QR code).
            )
            ...
            reportWithFields = self.client.service.GetVerificationReport(
                id, reportOptions
            )
            ...
            if e.fault.faultcode == "a:InvalidArgumentException":
                raise Exception(
                    "The document does not have a report/closed report, or None is passed as 'id' in GetVerificationReport: "
                    + e.fault.faultstring
                )
            if e.fault.faultcode == "a:DocumentIdException":
                raise Exception(
                    "Specified invalid 'DocumentId'" + e.fault.faultstring
                )
            raise
...
class AsyncAntiplagiatClient:
    ...
    async def simple_check(
        ...
        data = await self._get_doc_data(
            filename, external_user_id=external_user_id
        )
        ...
        try:
            uploadResult = await self.client.service.UploadDocument(
                data, docatr
            )
        except Exception:
            raise
    ...
    async def get_verification_report_pdf(
        ...
        try:
            ...
            reportWithFields = await self.client.service.GetVerificationReport(
                id, reportOptions
            )
            # No decoding needed.
            # decoded = base64.b64decode(reportWithFields)
